...
Y, X = np.mgrid[-1.3:1.3:resolution, -2.5:2.5:resolution]
...
for i in range(200):
    ...
    zs_ = zs*zs + c
    # Have we diverged?
    not_diverged = torch.abs(zs_) < 100.0 # Any greater and it has diverged
    # Update variables
    ns += not_diverged
    zs = zs_
    t2 = time.time()
...
plt.savefig("figures/part2_julia_hd.png")
plt.show()